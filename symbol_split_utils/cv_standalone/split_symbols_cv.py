...
def merge_close_rects(rects, merge_step_pixels=20):
    """
      Слияние близких боксов
      
      Боксы, разница между координатами вершин которых меньше порога,
      будут слиты в один бокс
      
      @rects - боксы символов
      @merge_step_pixels - порог слияния
    """
    if not len(rects):
        return []
...
def filter_rects_wh(rects, max_wh_ratio=1.5, min_h=None):
    """
      Фильтрация боксов по соотношению ширины и высоты
      
      @rects - боксы символов
      @max_wh_ratio - максимальное отношение ширины к высоте
      @min_h - Минимальная высота символа
    """
...
def filter_rects_variance(img, rects, min_var):
    """
      Фильтрация по вариации внутри бокса
      
      @img - изображение
      @rects - боксы символов
      @min_var - минимальная вариация в боксе
    """
...
def detect_text_cv(img, nm1_path, nm2_path, min_variance=200,
    ...
    """ 
      ...
      Алгоритм:
        - Детектирование регионов с помощью cv2.text (NM1 и NM2)
        http://docs.opencv.org/3.0-beta/modules/text/doc/erfilter.html
        - Взятие ограничивающих прямоугольников для регионов
        - Фильтрация полученных боксов по вариации, отношению ширины к высоте
        и вложенности в другие боксы
